...
void InitializeForcing(void) {
	...
	//--------------------------------- Apply Body Forcing 
	else if(!(strcmp(sys_vars->forcing, "BODY_FORC_SIN")) || !(strcmp(sys_vars->forcing, "BODY_FORC_COS")) || !(strcmp(sys_vars->forcing, "ISO_BODY_FORC"))) { 
		// BODY_FORC_SIN -> f_omega(x, y) = sin(2y) -> see Keeyeol Nam,  Edward Ott, Thomas M. Antonsen, Jr., and Parvez N. Guzdar, Phys. Rev Letters, 2000
		...
		// ISO_BODY_FORC -> Combo of the two
		// note sys_vars->forc_k will be period of the forcing (= 2 in the paper)
		
		// Loop through modes to identify local process(es) containing the modes to be forced
		if(!(strcmp(sys_vars->forcing, "BODY_FORC_SIN"))) {
			for (int i = 0; i < sys_vars->local_Ny; ++i) {
				if (run_data->k[0][i] == sys_vars->force_k || run_data->k[0][i] == -sys_vars->force_k) { 
					sys_vars->local_forcing_proc = 1;
					num_forced_modes++;
			...
			for (int j = 0; j < Nx_Fourier; ++j) {
				if (run_data->k[1][j] == sys_vars->force_k && run_data->k[0][0] == 0.0) { 
					sys_vars->local_forcing_proc = 1;
					num_forced_modes++;
				}
			} 
		...
		if (sys_vars->local_forcing_proc) {
			...
			if(!(strcmp(sys_vars->forcing, "BODY_FORC_SIN"))) {
				...
				for (int j = 0; j < Nx_Fourier; ++j) {
					if (run_data->k[1][j] == sys_vars->force_k && run_data->k[0][0] == 0.0) { 
						// Get the forcing info
						run_data->forcing_indx[force_mode_counter] = j;
...
void ComputeForcing(double dt) {
	...
	if (sys_vars->local_forcing_proc) {
		...
		//---------------------------- Compute Body Forcing Sin -> f_omega(x, y) = sin(2y); -> f_k = +/- (-i/2) * scale * \delta(n - 2); scale = 1 to match the paper
		else if(!(strcmp(sys_vars->forcing, "BODY_FORC_SIN"))) {
			// Compute the Body Forcing
			for (int i = 0; i < sys_vars->num_forced_modes; ++i) {
				if (run_data->forcing_k[0][i] == sys_vars->force_k) {
					run_data->forcing[i] = 0.0 - 0.5 * sys_vars->force_scale_var * I;
				}
				else { // Sign change for ky = - k_f
					run_data->forcing[i] = 0.0 + 0.5 * sys_vars->force_scale_var * I;	
				}
			}
		}
		...
		//---------------------------- Compute "Isotropic" Body Forcing -> f_omega(x, y) = cos(nx) + sin(ny);
		else if(!(strcmp(sys_vars->forcing, "ISO_BODY_FORC"))) {
			// Compute the Body Forcing
			for (int i = 0; i < sys_vars->num_forced_modes; ++i) {
				if (run_data->forcing_k[0][i] == sys_vars->force_k) {
				...
				else if (run_data->forcing_k[0][i] == -sys_vars->force_k) { // Sign change for ky = - k_f
					run_data->forcing[i] = 0.0 + 0.5 * sys_vars->force_scale_var * I;	
				}
				else { // forcing on mode (0, n)
					run_data->forcing[i] = 0.5 * sys_vars->force_scale_var + 0.0 * I;		
				}
			}
		}
