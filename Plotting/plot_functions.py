...
def plot_phase_snaps(out_dir, i, w, w_h, x, y, time, Nx, Ny, k2Inv, kx, ky):
    ...
    im3 = ax3.imshow(np.absolute(ZeroCentredField(w_h)) ** 2, extent = (Ny / 2, -Ny / 2 + 1, -Nx / 2 + 1, Nx / 2), cmap = mpl.colors.ListedColormap(cm.magma.colors[::-1]), norm = mpl.colors.LogNorm())
    ...
    im4 = ax4.imshow(np.absolute(ZeroCentredField(w_h * k2Inv.astype('complex128')))** 2, extent = (Ny / 2, -Ny / 2 + 1, -Nx / 2 + 1, Nx / 2), cmap = mpl.colors.ListedColormap(cm.magma.colors[::-1]), norm = mpl.colors.LogNorm())
