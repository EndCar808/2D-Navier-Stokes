...
if __name__ == '__main__':
    ...
    cmdargs.out_dir_stats_snaps = cmdargs.out_dir_stats + "SECT[{},{}]_KFRAC[{:1.2f},{:1.2f}]_TAG[{}]/".format(post_data.num_k3_sect, post_data.num_k1_sect, post_data.kmin_sqr, post_data.kmax_frac, cmdargs.tag)
    ...
    if os.path.isdir(cmdargs.out_dir_stats_snaps) != True:
        print("Making folder:" + tc.C + " SECT[{},{}]_KFRAC[{:1.2f},{:1.2f}]_TAG[{}]/".format(post_data.num_k3_sect, post_data.num_k1_sect, post_data.kmin_sqr, post_data.kmax_frac, cmdargs.tag) + tc.Rst)
        os.mkdir(cmdargs.out_dir_stats_snaps)
    ...
    my_ygb = mpl.colors.ListedColormap(cm.YlGnBu(np.arange(0,cm.YlGnBu.N)))
    my_ygb.set_under(color = "white")
    ...
    if hasattr(post_data, 'joint_enst_sync_hist_ranges_x'):
        ...
        im1 = ax1.imshow(np.rot90(pdf, k=1), extent=[xedges[0], xedges[-1], yedges[0], yedges[-1]], aspect='auto', cmap=my_ygb, norm = mpl.colors.LogNorm())
        ...
        cb1   = plt.colorbar(im1, cax = cbax1)
        cb1.set_label(r"PDF")
        plt.savefig(cmdargs.out_dir_stats_snaps + "/ConditionalSync_JointEnstrophySync_PDF.png", bbox_inches="tight")
        plt.close()
    ...
    if hasattr(post_data, 'sync_cond_max_norm'):
        ...
        ax1.set_xlabel(r"$t$")
        ax1.set_ylim(0-0.1, 1.1)
        ax1.legend()
        plt.savefig(cmdargs.out_dir_stats_snaps + "/ConditionalSync_MaxNorm.png", bbox_inches="tight")
        plt.close()

    ##------------------ Conditional Vorticity Incrment PDFs
    if hasattr(post_data, 'sync_cond_vort_incr_hist_counts'):
        
        for r_thres_indx, r_thres in enumerate([0.55, 0.65, 0.75, 0.85, 0.95]):
            for inc_indx, inc in enumerate([1, 2, 4, 8, sys_vars.Nx//2]):
                sync_cutoff = r_thres
                print(sync_cutoff, inc)
                fig = plt.figure(figsize = (16, 9))
                gs  = GridSpec(2, 2, hspace=0.25)
                ax1 = fig.add_subplot(gs[0, 0])
                unsync_ranges = post_data.sync_cond_vort_incr_hist_ranges[0, 0, inc_indx, r_thres_indx, :]
                unsync_hist   = post_data.sync_cond_vort_incr_hist_counts[0, 0, inc_indx, r_thres_indx, :]
                unsync_bin_cent, unsync_pdf = compute_pdf(unsync_ranges, unsync_hist, normalized = False)
                ax1.plot(unsync_bin_cent, unsync_pdf, label=r"$Q \leq {}$".format(np.around(sync_cutoff, 2)))
                # print(unsync_hist, unsync_ranges)
                sync_ranges = post_data.sync_cond_vort_incr_hist_ranges[1, 0, inc_indx, r_thres_indx, :]
                sync_hist   = post_data.sync_cond_vort_incr_hist_counts[1, 0, inc_indx, r_thres_indx, :]
                # print(sync_hist, sync_ranges)
                sync_bin_cent, sync_pdf = compute_pdf(sync_ranges, sync_hist, normalized = False)
                ax1.plot(sync_bin_cent, sync_pdf, label=r"$Q > {}$".format(np.around(sync_cutoff, 2)))
                all_ranges = post_data.sync_cond_vort_incr_hist_ranges[2, 0, inc_indx, r_thres_indx, :]
                all_hist   = post_data.sync_cond_vort_incr_hist_counts[2, 0, inc_indx, r_thres_indx, :]
                # print(all_hist, all_ranges)
                all_bin_cent, all_pdf = compute_pdf(all_ranges, all_hist, normalized = False)
                ax1.plot(all_bin_cent, all_pdf, label=r"$Q \in [0, 1]$")
                ax1.set_yscale('log')
                ax1.set_ylabel(r"PDF")
                ax1.legend()
                # ax1.set_xlabel(r"$\delta \omega (r) / \langle \delta \omega (r)^2\rangle^{1/2}$")
                ax1.set_xlabel(r"$\delta \omega (r) $")
                ax1.set_ylabel(r"PDF")
                ax1.grid()
                ax1.set_title("Longitudinal Increments")
                ax1 = fig.add_subplot(gs[0, 1])
                unsync_ranges = post_data.sync_cond_vort_incr_hist_ranges[0, 1, inc_indx, r_thres_indx, :]
                unsync_hist   = post_data.sync_cond_vort_incr_hist_counts[0, 1, inc_indx, r_thres_indx, :]
                bin_cent, pdf = compute_pdf(unsync_ranges, unsync_hist, normalized = False)
                ax1.plot(bin_cent, pdf, label=r"$Q \leq {}$".format(np.around(sync_cutoff, 2)))
                sync_ranges = post_data.sync_cond_vort_incr_hist_ranges[1, 1, inc_indx, r_thres_indx, :]
                sync_hist   = post_data.sync_cond_vort_incr_hist_counts[1, 1, inc_indx, r_thres_indx, :]
                bin_cent, pdf = compute_pdf(sync_ranges, sync_hist, normalized = False)
                ax1.plot(bin_cent, pdf, label=r"$Q > {}$".format(np.around(sync_cutoff, 2)))
                all_ranges = post_data.sync_cond_vort_incr_hist_ranges[2, 1, inc_indx, r_thres_indx, :]
                all_hist   = post_data.sync_cond_vort_incr_hist_counts[2, 1, inc_indx, r_thres_indx, :]
                bin_cent, pdf = compute_pdf(all_ranges, all_hist, normalized = False)
                ...
                unsync_ranges = post_data.sync_cond_vort_incr_hist_ranges[0, 0, inc_indx, r_thres_indx, :]
                unsync_hist   = post_data.sync_cond_vort_incr_hist_counts[0, 0, inc_indx, r_thres_indx, :]
                unsync_bin_cent, unsync_pdf = compute_pdf(unsync_ranges, unsync_hist, normalized = True)
                ax1.plot(unsync_bin_cent, unsync_pdf, label=r"$Q \leq {}$".format(np.around(sync_cutoff, 2)))
                # print(unsync_hist, unsync_ranges)
                sync_ranges = post_data.sync_cond_vort_incr_hist_ranges[1, 0, inc_indx, r_thres_indx, :]
                sync_hist   = post_data.sync_cond_vort_incr_hist_counts[1, 0, inc_indx, r_thres_indx, :]
                # print(sync_hist, sync_ranges)
                sync_bin_cent, sync_pdf = compute_pdf(sync_ranges, sync_hist, normalized = True)
                ax1.plot(sync_bin_cent, sync_pdf, label=r"$Q > {}$".format(np.around(sync_cutoff, 2)))
                all_ranges = post_data.sync_cond_vort_incr_hist_ranges[2, 0, inc_indx, r_thres_indx, :]
                all_hist   = post_data.sync_cond_vort_incr_hist_counts[2, 0, inc_indx, r_thres_indx, :]
                # print(all_hist, all_ranges)
                all_bin_cent, all_pdf = compute_pdf(all_ranges, all_hist, normalized = True)
                ax1.plot(all_bin_cent, all_pdf, label=r"$Q \in [0, 1]$")
                ax1.set_yscale('log')
                ax1.set_ylabel(r"PDF")
                ax1.legend()
                ...
                ax1.grid()
                ax1.set_title("Tansverse Increments")
                plt.suptitle("Increment $r = {} R_threshold = {}$".format(inc, r_thres))
                plt.savefig(cmdargs.out_dir_stats_snaps + "/ConditionalSync_SyncCond_PDF_ALL_Incr[{}]_Rthresh[{}].png".format(int(inc), r_thres), bbox_inches="tight")
                plt.close()
                ...
                ax1.set_title("Tansverse Increments")
                plt.suptitle("Increment $r = {} R_threshold = {}$".format(inc, r_thres))
                plt.savefig(cmdargs.out_dir_stats_snaps + "/ConditionalSync_SyncCond_PDF_Incr[{}]_Rthresh[{}].png".format(int(inc), r_thres), bbox_inches="tight")
                plt.close()
        ...
        ax1 = fig.add_subplot(gs[0, 0])
        for inc_indx, inc in enumerate([1, 2, 4, 8, sys_vars.Nx//2]):
            all_ranges = post_data.sync_cond_vort_incr_hist_ranges[2, 0, inc_indx, 0, :]
            all_hist   = post_data.sync_cond_vort_incr_hist_counts[2, 0, inc_indx, 0, :]
            # print(all_hist, all_ranges)
            all_bin_cent, all_pdf = compute_pdf(all_ranges, all_hist, normalized = False)
            ax1.plot(all_bin_cent, all_pdf, label = r"$r = {}$".format(inc))
            ax1.set_yscale('log')
            ax1.set_ylabel(r"PDF")
            ax1.legend()
            # ax1.set_xlabel(r"$\delta \omega (r) / \langle \delta \omega (r)^2\rangle^{1/2}$")
        ...
        ax1 = fig.add_subplot(gs[0, 1])
        for inc_indx, inc in enumerate([1, 2, 4, 8, sys_vars.Nx//2]):            
            all_ranges = post_data.sync_cond_vort_incr_hist_ranges[2, 1, inc_indx, 0, :]
            all_hist   = post_data.sync_cond_vort_incr_hist_counts[2, 1, inc_indx, 0, :]
            all_bin_cent, all_pdf = compute_pdf(all_ranges, all_hist, normalized = True)
            ax1.plot(all_bin_cent, all_pdf, label = r"$r = {}$".format(inc))
        ax1.set_yscale('log')
        ax1.set_ylabel(r"PDF")
        ...
        ax1.grid()
        ax1.set_title("Tansverse Increments")
        plt.suptitle("Vorticity Increment PDFs")
        plt.savefig(cmdargs.out_dir_stats_snaps + "/ConditionalSync_IncrPDF_All.png", bbox_inches="tight")
        plt.close()
