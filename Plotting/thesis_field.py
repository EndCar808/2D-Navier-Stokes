...
if __name__ == '__main__':
	...
	im1  = ax1.imshow(enst_spectrum[t1_w, :, :], extent = (-sys_vars.Ny / 3 + 1, sys_vars.Ny / 3, -sys_vars.Nx / 3 + 1, sys_vars.Nx / 3), cmap = my_magma, norm = mpl.colors.LogNorm()) # vmin = spec_lims[3], vmax = spec_lims[2]   # extent = (-sys_vars.Ny / 3 + 1, sys_vars.Ny / 3, -sys_vars.Nx / 3 + 1, sys_vars.Nx / 3), cmap = mpl.colors.ListedColormap(cm.magma.colors[::-1]), 
	...
	cb1.set_label(r"$\mathcal{E}(\hat{\omega}_\mathbf{k})$", fontsize= lab_size)
	ax2  = fig.add_subplot(gs[0, 1])
	im2  = ax2.imshow(enst_spectrum[t2_w, :, :], extent = (-sys_vars.Ny / 3 + 1, sys_vars.Ny / 3, -sys_vars.Nx / 3 + 1, sys_vars.Nx / 3), cmap = my_magma, norm = mpl.colors.LogNorm()) # vmin = spec_lims[3], vmax = spec_lims[2]   # extent = (-sys_vars.Ny / 3 + 1, sys_vars.Ny / 3, -sys_vars.Nx / 3 + 1, sys_vars.Nx / 3), cmap = mpl.colors.ListedColormap(cm.magma.colors[::-1]), 
	...
	cb2.set_label(r"$\mathcal{E}(\hat{\omega}_\mathbf{k})$", fontsize= lab_size)
	...
	ax3.plot(time, np.absolute(enst_flux_50), label = r"$\Pi_{50}(t)$")
	ax3.plot(time, np.absolute(enst_flux_100), label = r"$\Pi_{75}(t)$")
	print(enst_flux_50)
	print(enst_flux_100)
	...
	# im1 = ax1.imshow(grad_sqr_msr, extent = (y[0], y[-1], x[-1], x[0]), cmap = "bone")
	# # im1 = ax1.imshow(grad_sqr_msr / np.sum(grad_sqr_msr), extent = (y[0], y[-1], x[-1], x[0]), cmap = "bone", norm = mpl.colors.LogNorm())
	# ax1.set_ylabel(r"$y$", fontsize=lab_size)
	# ax1.set_xlabel(r"$x$", fontsize=lab_size)
	# ax1.set_xlim(0.0, y[-1])
	# ax1.set_ylim(0.0, x[-1])
