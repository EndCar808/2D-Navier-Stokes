...
do 
	...
	do
		...
		## Post command
		post_command="PostProcessing/bin/PostProcess_phase_sync -i $data_dir -o $data_dir -v 5e-13 -v 1 -v 4.0 -d 0.1 -d 1 -d 0.0 -f BODY_FORC_COS -f 2 -f 1.0 -t $tag -a $sec -a $sec -k $c_rad -k $kmin -p 1 -p 1"
		echo -e "\nCommand run:\n\t \033[1;36m $post_command \033[0m"
		$post_command &
	done
