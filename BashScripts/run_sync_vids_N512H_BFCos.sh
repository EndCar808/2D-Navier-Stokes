...
do 
	...
	do
		...
		data_dir=$input_dir"NAV_AB4_FULL_N[$N,$N]_T[0.0,0.0005,"$T"]_NU[5e-18,1,4.0]_DRAG[0.1,1,0.0]_FORC[BODY_FORC_COS,2,1]_u0[RANDOM_ENRG]_TAG[$solv_tag]/"

		## Solver command
		post_command="mpirun -n $num_procs Solver/bin/solver -o $input_dir -n $N -n $N -s 0.00000 -e $T -T 1 -T $trans_frac -c 1 -c 0.900000 -h 0.0005000 -h 0 -v 5e-18 -v 1 -v 4.0 -d 0.1 -d 1 -d 0.0 -i RANDOM_ENRG -t $solv_tag -f BODY_FORC_COS -f 2 -f 1.000 -p $save"
