...
int main(int argc, char** argv) {
	...
	// Get parllel time
	start = omp_get_wtime();
	#pragma omp parallel num_threads(num_threads) shared(data_u, str_func_par) private(tmp, indx, r_tmp, r_indx, x_indx, y_indx)
	{
		#pragma omp single 
		{			
			// Loop over powers
	...
	for (int p = 1; p <= NUM_POW; ++p) {
		...
		printf("str_func[%d]: %16.16lf - %16.16lf | %16.16lf - %16.16lf\t\t | %lf \t %lf\n", p, ser_num / Max_Incr, par_num / Max_Incr, ser_num / (Max_Incr * Max_Incr), par_num / (Max_Incr * Max_Incr), ser_num - par_num, ser_rad_num - par_rad_num);
	}
