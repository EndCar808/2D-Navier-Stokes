					...
					for (int r = 1; r <= Max_Incr; ++r) {
						...
						for (int i = 0; i < Ny; ++i) {
							for (int j = 0; j < Nx; ++j) {
								...
								increment += pow(fabs(data_u[SYS_DIM * ((((i + r) % Ny) * Nx + j)) + 1] - data_u[SYS_DIM * indx + 1]), p);
							}
						}
